...
public class DecryptBlindShuffleEvents extends AUsecase<EventBatch, EventBatch> {
  ...
  @Override
  public EventBatch runUsecase(EventBatch eventBatch) throws MPCException {
    ...
    return shuffleEvents.runUsecase(
        EventBatch.builder().events(outputEvents).eventBatchPairId(eventBatch.getEventBatchPairId())
            .build());
  }
}
